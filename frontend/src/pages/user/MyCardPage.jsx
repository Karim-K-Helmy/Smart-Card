...
export default function MyCardPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [{ data: cardResponse }, previewResponse] = await Promise.all([
          getMyCard(),
          getMyCardPreview(),
        ]);
          ...
          <div className="stack-md">
            <div className="smart-card-preview" style={{ minHeight: 420, justifyContent: 'center' }}>
              {previewUrl ? (
                ...
                <p>{status.loading ? 'جاري تحميل المعاينة...' : 'لا توجد معاينة متاحة'}</p>
              )}
            </div>
            <Button onClick={handleDownloadPdf} disabled={isDownloading || status.loading || !card}>

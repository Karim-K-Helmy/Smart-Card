...
function buildContactItems({ user }) {
  ...
  if (user.whatsappNumber) {
    ...
    items.push({ key: 'whatsapp', value: user.whatsappNumber, href: `https://wa.me/${sanitized}`, icon: '💬', bg: '#25D366' });
  }
...
export default function PublicProfilePage() {
  ...
  const personalBio = user.bio || personal.aboutText || '';
  ...
  return (
    <div className="star-wrapper">
      ...
      <style>{`
        ...
        .s-container { max-width: 430px; width: 100%; margin: 0 auto; }
        ...
        .s-link-button { background: #fff; border-radius: 15px; padding: 16px 20px; margin-bottom: 12px; display: flex; align-items: center; text-decoration: none; color: #333; transition: .3s ease; box-shadow: 0 4px 10px rgba(0,0,0,.15); }
        .s-link-button:hover { transform: translateY(-3px); box-shadow: 0 6px 15px rgba(0,0,0,.25); }
        ...
        .s-link-action { width: 30px; display: flex; align-items: center; justify-content: center; font-size: 18px; color: #666; }
        ...
        .bc-accordion { max-height: 0; overflow: hidden; transition: max-height .35s ease, padding-top .35s ease; }
        .bc-accordion.open { max-height: 520px; padding-top: 18px; }
        ...
        .s-omar-logo { width: 90px; height: 90px; border-radius: 50%; border: 4px solid #fff; background: #000; margin: 0 auto 15px; display: flex; align-items: center; justify-content: center; position: relative; z-index: 1; }
        .s-omar-logo-text { font-size: 24px; font-weight: bold; background: linear-gradient(135deg, #ff6b9d, #8b5cf6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        ...
        .s-footer-card { background: #fff; border-radius: 12px; padding: 15px; text-align: center; color: #333; font-size: 13px; line-height: 1.6; margin-top: 20px; font-weight: 600; }
      ...
      <div className="s-container">
        ...
        {contactItems.map((item) => (
          <a href={item.href} target="_blank" rel="noreferrer" className="s-link-button" key={item.key}>
            <div className="s-link-icon" style={{ background: item.bg, color: '#fff' }}>{item.icon}</div>
            <div className="s-link-text">{item.value}</div>
            <div className="s-link-action">{item.icon}</div>
          </a>
        ))}
                  ...
                  <div className="bc-card-footer">
                    ...
                    <div className={`bc-accordion ${isExpanded ? 'open' : ''}`}>
                      <div className="bc-details-grid">
                        ...
                        {actions.length ? <div className="bc-detail-item"><div className="bc-detail-label">روابط سريعة</div><div className="bc-link-pills">{actions.map((action) => <a key={action.key} href={action.href} target="_blank" rel="noreferrer" className="bc-link-pill">{action.label}</a>)}</div></div> : null}
                ...
                .map((product) => {
                  ...
                  return (
                    <div key={product._id} className="store-card-container">
                      ...
                      <div className="bc-card-footer">
                        {hasName ? <div className="product-name-label">اسم العمل / المنتج</div> : null}
        ...
        <div className="s-omar-card">
          <div className="s-omar-logo"><div className="s-omar-logo-text">OMAR</div></div>
          <div className="s-omar-title">One Minute Ad Reel</div>
          <div className="s-omar-subtitle">Smart Digital Presence</div>
        </div>
        <div className="s-footer-card">تواصل معنا لنصمم لك على طريقتك الخاصة، تميز بعرض نشاطك الاحترافي</div>
      </div>
